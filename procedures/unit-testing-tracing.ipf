...
static Constant MAX_PROC_LINES = 10000
static StrConstant PROC_BACKUP_ENDING = ".traced"
...
static Function SetupTracing(string procWinList, string traceOptions)
	...
	variable instrumentOnly, noRTFCreation
...
static Function AfterCompiledHook()
	...
	if(!NVAR_Exists(ACHookEnabled) || (NVAR_Exists(ACHookEnabled) && !ACHookEnabled))
		return NaN
	endif
	...
	RunTest("", traceWinList=IUTF_TRACE_REENTRY_KEYWORD)
End

static Function/S GetTaggedFunctionName(string procWin)

	return FUNCTION_TAG_PREFIX + Hash(procWin, 1)
End
...
static Function SetupTraceProcedures(string procWinList, string traceOptions)
	...
	for(i = 0; i < numProcs; i += 1)
		procWin = StringFromList(i, procWinList)
		[procText, funcPath] = AddTraceFunctions(procWin, i, traceOptions)
		if(!UTF_Utils#IsEmpty(funcPath))

			Open/R/Z fNum as funcPath
			if(V_flag)
				Abort "Open failed:" + funcPath
			endif
			FStatus fNum
			...
			if(V_flag)
				Abort "Open failed:" + (funcPath + PROC_BACKUP_ENDING)
			endif
...
static Function WriteProcList(string procWinList)
	...
	if(err)
		Abort "Unable to retrieve full function name."
	endif
...
static Function/WAVE GetFunctionDeclarationList(string line)
	...
	b1 = strsearch(line, "(", 0)
	if(b1 < 0)
		Abort "Error parsing function declaration: " + line
	endif
...
static Function [WAVE/T w, string funcPath_] AddTraceFunctions(string procWin, variable procNum, string traceOptions)

	string allProcWins, errMsg
	string funcList, fullFuncName, funcName, funcPath
	string line, preLine, origLines, preFuncLines
	string newLine, newProcCode, sTmp, exclList
	...
	exclList = LowerStr(StringByKey(UTF_KEY_NOINSTR, traceOptions))
	...
	for(i = 0; i < numFunc; i += 1)
		...
		funcExclusionFlag[i] = WhichListItem(LowerStr(fullFuncName), exclList, ",") != -1
		if(UTF_Utils#isEmpty(funcPath))
			funcPath = FunctionPath(fullFuncName)
		endif
	endfor
	...
	return [ListToTextWave(newProcCode, "\r"), funcPath]
End
...
Function/T ReplaceWithZ(string &origLines, variable currLineNum, variable &lineCnt, variable procNum)
	...
	if(b1 == -1 || b2 == -1)
		Abort "Failed to parse condition;" + origLines
	endif
...
static Function/S GetLineEnding(string line[, string defEndL])
	...
	if(UTF_Utils#IsEmpty(line))
		...
		Abort "Can not determine line ending."
	endif
...
static Function AnalyzeTracingResult()
	...
	string funcList, fullFuncName, procWin, funcPath, procText, prefix, line
	...
	for(i = 0; i < numProcs; i += 1)
		...
		for(j = 0; j < numProcLines; j += 1)
			...
			if(!(execC + nobranchC + branchC))
				prefix = "________|________|________|" + wProcText[j] + "\r"
				Notebook NBTracedData selection={endOfFile, endOfFile}, text=prefix
				Notebook NBTracedData selection={startOfPrevParagraph, endOfPrevParagraph}, textRGB=(0, 0, 0)
				continue
			endif
			...
			prefix +=  wProcText[j] + "\r"
			Notebook NBTracedData selection={endOfFile, endOfFile}, text=prefix
			Notebook NBTracedData selection={startOfPrevParagraph, endOfPrevParagraph}, textRGB=(0, 50 * 256, 204 * 256)
		endfor
		SaveNotebook/O/P=home/S=4 NBTracedData as (procWin + ".rtf")
	endfor
End
